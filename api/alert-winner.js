    ...
    try {
        ...
        if (claims.rows.length === 1) {
            ...
            if (simultaneousClaims.length === 1) {
                ...
                // Truly simultaneous - pick randomly but deterministically
                // Use faction_id + alert_type as seed for consistency
                const seed = parseInt(factionId) + alertType.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
                const randomIndex = seed % simultaneousClaims.length;
        ...
        return res.json({
            ...
            resolutionMethod: claims.rows.length === 1 ? 'only_claim' : 
                             simultaneousClaims.length === 1 ? 'timestamp' : 'random',
            message: isWinner 
                ? 'You won! Send the alert.'
                : `Another client will handle this alert.`
        });
